...
| モデル | D1 Rules↑ | D2 Spatial↑ | D3 Tactical↑ | **SUS↑** | 備考 |
|---|---|---|---|---|---|
| LLaMA-3-8B-Instruct | 50.0% | 60.0% | 66.7% | **57.0%** | Judge として現行使用中 |
| LLaMA-3.1-8B-Instruct | - | - | - | - | 申請中（2026-05-25） |

### D1 個別結果（LLaMA-3-8B）
- Q1 バックパス間接FK: ✗（"free kick" とは言ったが "indirect" なし）
- Q2 スローイン・オフサイド例外: ✗（誤ってオフサイドと判定）
- Q3 ペナルティエリアライン上のファウル: ✓
- Q4 ゴールキック→自陣ゴール: ✓

### D2 個別結果（LLaMA-3-8B）
- Q1 4-1-4-1系: ✓（4-3-3と回答、許容範囲内）
- Q2 3-4-2-1系: ✗（3-2-3-1と回答）
- Q3 4-1-2-1-2: ✓
- Q4 4-2-3-1: ✓
- Q5 3-1-4-2系: ✗（3-1-2-2-2と回答）

### 考察
- **D3 Tactical（66.7%）が最も高い** → 戦術意図・プレー説明の Judge として一定の信頼性あり
- **D1 Rules（50%）に穴** → スローインのオフサイド例外など細かいルールに誤り
- **D2 Spatial（60%）** → 3バック系フォーメーションの空間認識が苦手
- 総合 SUS=60.7% は「合格ライン上」。commentary・intent タスクの Judge としては使用可能だが、formation タスクには限界がある
