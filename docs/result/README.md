...
>   - △A = action クエリでは正確だが異ドメイン指示に沈黙・テンプレート固定
>   - △B = 指示の形式には反応できるが内容は LLM 事前知識による幻覚
>   - × = ほぼ全件崩壊（空文字・echo・ガーベジ・無限ループ）

| 実験 | LoRA | ep | Phase 2 F1↑ | Phase 3 F1↑ | Free QA | 備考 |
|---|---|---|---|---|---|---|
| 指示文なし + 自然文 (202605051310) | なし | 10 | 0.7466 | 0.0000 | × | 推論時に指示文が来ると全件空文字 |
| 指示文あり + 自然文 (202605051303) | なし | 10 | 0.7166 | 0.8371 | △A | 全件生成・テンプレート固定・指示内容無視 |
