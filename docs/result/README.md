...
| No LoRA + 指示+回答多様化 (202605062134) | なし | 10 | 0.6615 | 0.7915 | △A | 出力フォーマット多様化・異ドメイン指示に未対応 |
...
**結論**: 指示文あり単体（202605051303）は Phase 3 F1=0.8371 と高いが Phase 4 では全件定型文／空文字で Free QA ×。No LoRA + 指示多様化（202605061813）が唯一 △A（first_action に 1 件以上意味のある応答あり）。LoRA あり／なしのトレードオフは多様化を加えても解消されない → Token-Word Alignment（B-3）が本命。
