...
| No LoRA + 指示多様化 50試合 (202605081047) | なし | 20 | 0.6137 | 0.7582 | × | formation/first_action=全件空文字・commentary=全件定型文。5試合△Aから悪化 |
...
| **Approach A: Phase1.5 InfoNCE → Phase2 (202605112350)** | なし | 10 | **0.780** | **0.780** | × | Phase2 train=0.262/val=0.419(ep10)。安定・過学習なし。formation/first_action全件空文字・commentary 15/20テンプレート固定 |
| **Approach B: Phase2 Encoder解凍+slot_loss (202605112330)** | なし | 10 | **0.847** | **0.847** | × | ep4(val=0.395)以降過学習(train→0.015/val→0.770)。formation/first_action全件空文字・commentary 18/20テンプレート固定 |
...
## Phase 2/3/4: Approach A (Phase1.5 InfoNCE → Phase2) / Approach B (Phase2 Encoder解凍+slot_loss)

> 設定: SENTENCE_FORMAT=1 / INSTRUCTION_DIVERSE=0 / LoRA なし / 5ゲーム / 10エポック  
> Approach A RUN_TS=202605112350 / Approach B RUN_TS=202605112330

| 指標 | Approach A | Approach B |
|---|---|---|
| Phase 2 Test F1↑ | 0.780 | **0.847** |
| Phase 3 F1↑ (n=15) | 0.780 | **0.847** |
| formation 非空出力 | 0/20 | 0/20 |
| commentary 非空出力 | 15/20 | 18/20 |
| first_action 非空出力 | 0/20 | 1/20 |
| Free QA 総合 | × | × |

**Phase 4 生成例（Approach A・commentary）**:
```
"In this soccer sequence, performing trap, pass and through pass."
"In this soccer sequence, performing pass, trap, foul and foul received."
"InParameter, trap, pass, shot and goalkeeper save."  ← 1件だけトークン混入
```

**Phase 4 生成例（Approach B・commentary）**:
```
"In this soccer sequence, performing pass, touch and clearance."
"In this soccer sequence, performing throw-in, trap and pass."
" In this soccer sequence, performing pass, trap, foul and foul received."
```

**考察**:
- Approach B は Phase 2/3 F1=0.847 と本実験系列で最高 F1 を記録（Phase 2 Encoder 解凍 + slot_loss の効果）
- Approach A (InfoNCE 対照学習) も F1=0.780 と B-0a 最良構成 (0.7345) を上回る
- Free QA は両 Approach とも × — commentary のみ部分的に出力するが内容は学習テンプレート固定
- **formation / first_action の全件沈黙** は B-2 以前の No LoRA 実験と同パターン。Encoder の言語情報不足が引き続き Free QA を制約している
- Phase 3 F1 ≈ Phase 2 F1 であることから、Phase 2 学習に使った指示文と Phase 3 の評価指示文が同形式（SENTENCE_FORMAT=1）で一致しており、学習-推論のギャップがない

**結論**:
Encoder の言語情報化（Phase 1.5 InfoNCE / Encoder 解凍）は Phase 2/3 の F1 を向上させるが、Free QA の style adaptation 問題は未解決。
LoRA なし + Instruction Diversification (202605061813) の Free QA 評価 △A より改善せず、むしろ formation/first_action の沈黙が増えた。
**Phase 1.5 InfoNCE は Phase 2 の収束改善に有効** だが、Free QA のためにはさらに大規模な Encoder 事前学習か LoRA との組み合わせが必要。
